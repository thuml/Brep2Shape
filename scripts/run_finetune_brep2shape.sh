...
export DATASET_DIR="${DATASET_DIR:-/path/to/segmentation_data}"
export NUM_CLASSES="${NUM_CLASSES:-8}"

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3}"
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-brep2shape_finetuning_fusion_360_segmentation}"
...
# Keep the encoder architecture identical to the pretrained model.
...
export DESC="${DESC:-fusion_360_segmentation_topo${GRAPH_NUM_LAYERS}_geo${SURFACE_NUM_LAYERS}_seed${SEED}}"
...
if [ ! -f "$DATASET_DIR/datasplit.json" ]; then
  echo "Fine-tuning split not found: $DATASET_DIR/datasplit.json" >&2
  exit 1
fi
